        ...
        <table class="min-w-full divide-y divide-gray-200 border table-fixed">
            <thead>
                @php
                    ...
                    $tableHeader = ['No', 'Id', 'Nama', 'Jabatan', 'Role', 'Action']
                @endphp
            ...
            <tbody>
                @foreach ($listUser as $index => $item)
                    <tr>
                        ...
                        <td class="p-2 w-80 text-center">{{ $item['id'] }}</td>
                        <td class="p-2 w-80 text-center">{{ $item['name'] }}</td>
                        <td class="p-2 w-80 text-center">{{ $item['position'] }}</td>
                        <td class="p-2 w-20 text-center">{{ $item['role'] }}</td>
                        <td class="p-2 w-80 text-center">
                            <div class="w-full flex flex-row justify-center gap-4 noCopy">
                                <a href="/admin/user/detail?user={{ $item['id'] }}">
                                    <button class="
                                        border border-black
                                ...
                                <a href="/admin/user/update?user={{ $item['id'] }}">
                                    <button class="
                                        border border-black
                                        rounded-lg px-2 py-1 text-center
                                        hover:bg-slate-800 hover:text-slate-300
